  ...
  const columns: ColumnsType<ProductTableData> = [
    {
      ...
      render: (_, record) => (
        <div className="flex items-center gap-2">
          <Avatar
            shape="square"
            src={record.primary_media_url ? record.primary_media_url : `https://placehold.co/150x150/E6F7FF/black?text=${record.name?.split(' ').map(word => word[0]).join('')}`}
            alt={record.name}
    ...
    },
    {
      ...
      width: 130,
      render: (_, record) => {
        return <Tag color={record.status === 'active' ? 'green' : 'red'} className="capitalize">{record.status || 'No status'}</Tag>;
      },
    },
    {
      title: 'Action',
